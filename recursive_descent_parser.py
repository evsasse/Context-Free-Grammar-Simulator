...
class RecursiveDescentParser():
	...
	def parse(self, sentence, log = None):
		# will maintain all the functions generated in this dict scope
		parser_wrapper = {}

		parser_wrapper['log'] = log

		exec(self.parser_code(log, True), parser_wrapper)
		return parser_wrapper['main'](sentence)


	def parser_code(self, log = None, log_on_code = False):
		if log != None and not log_on_code:
			log('Iniciando geração de código do parser')

		code = self._parser_code_main()
		code += self._parser_code_next_lexic_symbol()
		for nonterminal in sorted(self._grammar._nonterminals):
			code += self._parser_code_nonterminal(nonterminal,log, log_on_code).strip()+'\n\n'
		return code
...
def main(sentence):
	...
	_sentence = (sentence.strip() + ' $').strip().split(' ')
...
def next_lexic_symbol():
	...
	def _parser_code_nonterminal(self,nonterminal,log = None, log_on_code = False):
		code = '''def %s():\n'''%(nonterminal)
		...
		if log != None and not log_on_code:
			log('Gerando o código do não terminal "%s"'%(nonterminal))

		productions_of_nonterminal = sorted([p for p in self._grammar._productions if p.left == nonterminal])
		should_do_else = '&' not in self._grammar._first(nonterminal,None if log_on_code else log)

		if log_on_code:
			code += '''\tlog('Entrando no não terminal %s')\n'''%(nonterminal)
		...
		if len(productions_of_nonterminal) == 1 and productions_of_nonterminal[0].right == ['&']:
			code += "\tpass\n\n"
			if log_on_code:
				code += '''\tlog('Saindo do não terminal %s')\n'''%(nonterminal)
			return code
		...
		for production in productions_of_nonterminal:
			if production.right != ['&']:
				code += '''\t%s current_symbol in %s:\n'''%(if_or_elif,sorted(self._grammar._first(production.right,None if log_on_code else log)))
				if_or_elif = "elif"
